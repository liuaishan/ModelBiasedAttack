...
class COCODemo(object):
	...
	def run_on_opencv_image(self, image, y):
		...
		cnt = self.correct_num(predictions, y)
		return cnt
		
	def correct_num(self, predictions, y):
		cnt = []
		for i in range(len(predictions)):
			scores = predictions[i].get_field('scores')
			labels = predictions[i].get_field('labels')
			
			other_idx = ~torch.eq(labels, y[i])
			other_lables = labels[other_idx]
			other_scores = scores[other_idx]
			if len(other_scores) > 0:
				sub_id = torch.argmax(other_scores)
				sub_id = other_lables[sub_id]
				cnt.append(int(sub_id.numpy()))
			else:
				sub_id = 0
				cnt.append(0)
			#sub_score = other_scores[sub_id]
			#print(str(sub_id) + ' : ' + str(sub_score))
		...
		if test:
			...
			for i in range(len(predictions)):
				...
				if torch.sum(ori_scores > 0.6) > 0:
					ori_correct += 1
			return ori_correct, all_prob_mean / len(predictions)
