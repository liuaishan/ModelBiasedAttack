def compute_saliency_maps(X, y, model):
	...
	Input : 
	- X : Input images : Tensor of shape (N, 3, H, W), if not match use transpose
...
class ModelOutputs():
	...
	def __init__(self, model, target_layers):
		self.model = model
		self.feature_extractor = FeatureExtractor(self.model.feature_layer, target_layers)#features
...
if __name__ == '__main__':
	...
	cfg.merge_from_file('configs/predict.yaml')
	cfg.merge_from_list([])
	cfg.freeze()
	...
	grad_cam = GradCam(model = model, \
					target_layer_names = ["30"], use_cuda=True)

	# print(args.image_path)
	img = cv2.imread("../../134.png", 1)
	# print(img)
