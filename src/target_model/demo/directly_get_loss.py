...
class COCODemo(object):
	...
	def run_on_opencv_image(self, image, y):
		...
		loss = self.get_loss(predictions, y, [0 for i in range(len(y))], False)
		return loss
	
	def test(self, image, y, ori_label):
		"""
		Arguments:
			image (np.ndarray): an image as returned by OpenCV
		...
		"""
		predictions = self.compute_prediction(image)
		ori_correct, target_correct, target_prob_sum = self.get_loss(predictions, y, ori_label, True)
		return ori_correct, target_correct, target_prob_sum
		
	def get_loss(self, predictions, y, ori_label, test):
		if test:
			ori_correct, target_correct, target_prob_sum, all_prob_mean = [0, 0, 0, 0]
			for i in range(len(predictions)):
				scores = predictions[i].get_field('scores')
				labels = predictions[i].get_field('labels')
				if len(scores) > 0:
					all_prob_mean += torch.mean(scores)
				
				target_idx = torch.eq(labels, y[i])
				ori_idx = torch.eq(labels, ori_label[i])
				target_scores = scores[target_idx]
				ori_scores = scores[ori_idx]
				if len(target_scores) > 0:
					target_prob_sum += torch.mean(target_scores)
				if torch.sum(target_scores > 0.6) > 0:
					target_correct += 1
				if torch.sum(ori_scores > 0.6) > 0:
					ori_correct += 1
			return ori_correct, target_correct, all_prob_mean / len(predictions)
