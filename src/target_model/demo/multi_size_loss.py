...
class COCODemo(object):
	...
	def test(self, image, ori_label):
		...
		"""
		predictions = self.compute_prediction(image)
		ori_correct, top_3, top_5 = self.get_loss(predictions, ori_label, True)
		return ori_correct, top_3, top_5
	...
	def correct_num(self, image, y):
		...
		for i in range(len(predictions)):
			...
			if (max_label == y[i]):
				...
				correct.append(0)
		return correct, top_3, top_5, pred_label
	
	def generate_fake_image_with_max_score(self, image):
		predictions = self.compute_prediction(image)[0]
		loss = torch.Tensor(np.array([0]))
		...
		if len(scores) == 0:
			return 0,0,0
		top_score = scores[torch.argmax(scores)]
		top_label = labels[torch.argmax(scores)]
		idx = torch.eq(labels, top_label)
		...
		return loss, top_label.detach().numpy(), top_score
	
	def generate_fake_image(self, image, y):
		predictions = self.compute_prediction(image)
		loss = torch.Tensor(np.array([0]))
		top_1 = []
		for i in range(len(predictions)):
			scores = predictions[i].get_field('scores')
			labels = predictions[i].get_field('labels')
			top1 = labels[torch.argmax(scores)]
			top_1.append(top1.detach().numpy())
