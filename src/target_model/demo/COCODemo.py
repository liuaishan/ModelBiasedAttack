...
class COCODemo(object):
	...
	def run_on_opencv_image(self, image, y):
		...
		all_scores, bbox = self.get_all_scores_and_bbox(predictions, y)
		return all_scores, bbox

	def get_all_scores_and_bbox(self, predictions, y):
		scores = predictions[0].get_field('scores')
		labels = predictions[0].get_field('labels')
		#idx = labels.index(y[0])
		idx = torch.argmax(torch.eq(labels, y[0]))
		b = predictions[0].bbox
		bbox = predictions[0].bbox[idx]
		...
		for i in range(1, len(predictions)):
			...
			#idx = labels.index(y[i])
			idx = torch.argmax(torch.eq(labels, y[i]))
			tmp = predictions[i].bbox[idx]
			tmp = tmp.unsqueeze(0)
			bbox = torch.cat([bbox, tmp], 0)
